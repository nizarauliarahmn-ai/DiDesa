...
- **Whitespace & No-Wrapping pada Tabel**:
  - Semua sel tabel (`th`, `td`) dan elemen badge (seperti `DUSUN / RT / RW`, `TAHUN`, `NIK`, `STATUS`) WAJIB menggunakan kelas Tailwind `whitespace-nowrap`.
...
## 🏛️ 2. Standar Pengelolaan Bantuan Sosial Desa
...
- **Hentikan & Perpanjang Bantuan**:
  - Penghentian bantuan wajib mencatat **Tanggal & Bulan Resmi Penghentian** (format lengkap), **Alasan Utama**, serta **Catatan Rinci Tambahan Admin**.
...
2. **Supabase Multi-Tenant Safety**:
   - Selalu gunakan `tenant_id` pada meupun query Supabase (`.eq('tenant_id', tenantId)`).
