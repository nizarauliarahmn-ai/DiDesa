    ...
    <div className="max-w-7xl mx-auto pb-24 space-y-6">
      ...
      <div className="sticky top-16 z-40 bg-slate-50/60 backdrop-blur-xl pb-4 -mx-4 -mt-4 px-4 pt-4 md:-mx-6 md:-mt-6 md:px-6 md:pt-6 lg:-mx-8 lg:-mt-8 lg:px-8 lg:pt-8 border-b border-slate-200/50 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <button 
            onClick={onBack}
            className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 transition-all text-gray-600"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Detail Data Penduduk</h2>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={() => {
            ...
            className="px-5 py-2.5 rounded-lg bg-blue-600 text-white font-bold hover:bg-blue-700 shadow-sm transition-all flex items-center gap-2 text-sm"
          >
          ...
          <button 
            ...
            className="px-5 py-2.5 rounded-lg border border-emerald-700 text-emerald-700 font-bold hover:bg-emerald-50 transition-all flex items-center gap-2 text-sm shadow-sm bg-white"
          >
          ...
          {!isPending && (
            <>
              <button onClick={onEdit} className="px-5 py-2.5 rounded-lg bg-emerald-700 text-white font-bold hover:bg-emerald-800 shadow-sm transition-all flex items-center gap-2 text-sm">
                <Edit2 className="w-4 h-4" />
                Edit Data
              </button>
              <button onClick={handleMoveResident} className="px-5 py-2.5 rounded-lg bg-amber-50 border border-amber-200 text-amber-700 font-bold hover:bg-amber-100 shadow-sm transition-all flex items-center gap-2 text-sm">
                <ArrowRightLeft className="w-4 h-4" />
                Mutasi Warga
              </button>
              <button onClick={handleDeleteResident} className="px-5 py-2.5 rounded-lg bg-rose-50 border border-rose-200 text-rose-700 font-bold hover:bg-rose-100 shadow-sm transition-all flex items-center gap-2 text-sm">
              ...
              <button 
                ...
                className="px-5 py-2.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-700 font-bold hover:bg-slate-200 shadow-sm transition-all flex items-center gap-2 text-sm"
              >
                <X className="w-4 h-4" />
                Tandai Meninggal
              </button>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center relative overflow-hidden">
            ...
            <div className="relative inline-block mb-4 mt-2">
              {data?.photo ? (
                <img src={data.photo} alt={data.name} className="w-32 h-32 rounded-2xl mx-auto border-4 border-white shadow-md object-cover" />
              ) : (
                <div className={`w-32 h-32 rounded-2xl mx-auto border-4 border-white shadow-md flex items-center justify-center bg-gradient-to-b text-white ${data?.gender === 'Perempuan' ? 'from-pink-300 to-pink-400' : 'from-blue-300 to-blue-400'}`}>
                  <User className="w-16 h-16" fill="currentColor" />
                </div>
              )}
              <span className="absolute bottom-2 -right-2 bg-emerald-700 text-white text-[10px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-widest border-2 border-white shadow-sm">
                Aktif
              </span>
            </div>
            <h3 className="text-xl font-bold text-gray-900">{data?.name || "Ahmad Bukhori"}</h3>
            <p className="font-mono text-gray-500 text-sm mt-1">NIK: {data?.nik || "320412003840001"}</p>
            <div className="mt-3">
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 grid grid-cols-2 gap-4 text-left">
              <div>
                ...
                <p className="font-bold text-gray-900 text-sm">{data?.noKk || data?.no_kk || "320412008890001"}</p>
              </div>
              <div>
                <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Agama</p>
                <p className="font-medium text-gray-900 text-sm">{data?.religion || "Islam"}</p>
              </div>
            </div>
          </div>

          {/* Ringkasan */}
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 border-l-4 border-l-emerald-600 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="font-bold text-gray-900 text-lg">Biodata</h4>
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 text-sm">{data?.gender || "Laki-Laki"}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Golongan Darah</p>
                    <p className="font-medium text-gray-900 text-sm">{data?.bloodType || "O"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Pendidikan</p>
                    <p className="font-medium text-gray-900 text-sm">{data?.education || "S1 - Ekonomi"}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Pekerjaan</p>
                    <p className="font-medium text-gray-900 text-sm">{data?.job || "Karyawan Swasta"}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Nama Ayah Kandung</p>
                    <p className="font-medium text-gray-900 text-sm">{data?.fatherName || "Budi Santoso"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Nama Ibu Kandung</p>
                    <p className="font-medium text-gray-900 text-sm">{data?.motherName || "Ratna Sari"}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="font-bold text-gray-900 text-lg">Alamat & Domisili</h4>
              </div>
              <div className="space-y-4">
                <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 text-sm">{data?.rtRw || `${data?.rt || "004"} / ${data?.rw || "012"}`}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Desa</p>
                    <p className="font-medium text-gray-900 text-sm">{data?.desa || "Desa Sukamaju"}</p>
                  </div>
                </div>
                <div className="mt-4 p-3 bg-gray-50 rounded-xl flex items-center gap-3 border border-gray-100">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                  <p className="text-[11px] font-bold text-gray-600">Sesuai KTP & Domisili Terdaftar</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h4 className="font-bold text-gray-900 text-lg">Hubungan Keluarga</h4>
              </div>
              <span className="text-sm font-bold text-gray-500 font-mono">KK: {data?.noKk || data?.no_kk || "320412008890001"}</span>
            </div>
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center gap-4 relative z-10 p-4 rounded-xl border transition-all ${
                      isCurrent 
                        ? 'bg-emerald-50/40 border-l-4 border-l-emerald-600 border-emerald-100 shadow-sm cursor-default' 
                        : 'bg-white hover:bg-emerald-50/20 border-gray-100 hover:border-emerald-100 hover:shadow-md cursor-pointer active:scale-[0.99] group'
                    } ${index > 0 ? 'ml-4' : ''}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`font-bold text-sm truncate ${isCurrent ? 'text-emerald-800' : 'text-gray-900 group-hover:text-emerald-700'}`}>
                        {member.name}
                      </p>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h4 className="font-bold text-gray-900 text-lg">Riwayat Administrasi</h4>
              </div>
              <button 
                onClick={() => {
                  if (onSetPresetResident && onNavigateToTab) {
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 text-white font-bold text-xs hover:bg-blue-700 active:scale-95 transition-all shadow-sm"
              >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50/80">
                  <tr>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider rounded-l-lg">Tanggal</th>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Jenis Layanan</th>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Keterangan</th>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider text-right rounded-r-lg">Status</th>
                  </tr>
                </thead>
                  ...
                  {residentLetters.length > 0 ? (
                    residentLetters.map((letter) => (
                      <tr key={letter.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-4 py-4 text-sm font-medium text-gray-600">{letter.tanggal}</td>
                        <td className="px-4 py-4 text-sm font-bold text-gray-900">{letter.jenis}</td>
                        <td className="px-4 py-4 text-sm text-gray-600">{letter.keperluan}</td>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h4 className="font-bold text-gray-900 text-lg">Riwayat Bantuan Sosial</h4>
              </div>
              <button 
                onClick={() => {
                  setAidError("");
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-600 text-white font-bold text-xs hover:bg-amber-700 active:scale-95 transition-all shadow-sm"
              >
                <Plus className="w-3.5 h-3.5" />
                Kelola Bantuan
              </button>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50/80">
                  <tr>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider rounded-l-lg">Tanggal</th>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Jenis Bantuan</th>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Keterangan</th>
                    <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider text-right rounded-r-lg">Status</th>
                  </tr>
                </thead>
                  ...
                  {data?.activeAids && data.activeAids.length > 0 ? (
                    data.activeAids.map((aid: string, idx: number) => (
                      <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-4 py-4 text-sm font-medium text-gray-500">Aktif</td>
                        <td className="px-4 py-4 text-sm font-bold text-gray-900">{aid}</td>
                        <td className="px-4 py-4 text-sm text-gray-600">Terdaftar sebagai penerima aktif</td>
                        <td className="px-4 py-4 text-right">
                    ...
                    <tr className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-4 text-sm font-medium text-gray-500">-</td>
                      ...
                      <td className="px-4 py-4 text-right">
                        <span className="px-2.5 py-1 rounded-md bg-gray-50 text-gray-500 text-[10px] font-bold uppercase tracking-wider border border-gray-100">NIHIL</span>
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {showAidModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-300 ease-out">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-xl border border-slate-100 p-6 animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-900">Kelola Bantuan Sosial</h3>
              <button 
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs font-mono text-gray-500">{data?.nik}</p>
              </div>
                ...
                {ALL_AID_PROGRAMS.filter(p => !data?.activeAids?.includes(p)).length > 0 ? (
                  <div className="flex gap-2">
                    <select
                      ...
                      className="flex-1 bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 outline-none"
                    >
                    ...
                    <button
                      ...
                      disabled={isUpdatingAid}
                      className="bg-emerald-700 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-sm hover:bg-emerald-800 active:scale-95 transition-all flex items-center gap-1 disabled:opacity-50"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      Tambah
                    </button>
