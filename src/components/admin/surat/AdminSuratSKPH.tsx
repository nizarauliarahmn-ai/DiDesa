...
import { User, FileSignature, AlertCircle, History, Trash2,
...
export default function AdminSuratSKPH({ 
  ...
  const backdateKlas = getLetterClassifications().find(c => c.klasifikasi === 'SKPH') || { klasifikasi: 'SKPH', kodeKlasifikasi: '400' };
  const kodeKlasifikasiSKPH = backdateKlas.kodeKlasifikasi || '400';
  ...
  const handleSave = async () => {
    ...
    try {
      if (editLetterId) {
        ...
        if (!isBackdate) incrementSequenceNumber('SKPEN');
...
const handlePrint = async (skipCheck = false) => {
    ...
    if (editLetterId) {
      ...
      if (!isBackdate) incrementSequenceNumber('SKPH');
    }
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <SuratEditorHeader template={getLetterHeaderTemplate('SKPH', { kode: '400', jenis: 'Surat Keterangan Penghasilan', deskripsi: 'Surat Keterangan Penghasilan / Gaji (SKPH)', nomorSurat: formData.nomorSurat })}
