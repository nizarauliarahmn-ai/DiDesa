...
import { 
  Store, User, Calendar, FileText, FileSignature, 
  AlertCircle, History, ZoomIn, ZoomOut
} from 'lucide-react';
...
export default function AdminSuratSKU({ 
  ...
  const kodeKlasifikasiSKU = backdateKlas.kodeKlasifikasi || '400';
  ...
  const [formData, setFormData] = useState({
    ...
    rw: '001',
    alamat: '',
    keperluan: 'Kelengkapan Pengajuan Tambahan Modal KUR Mikro',
    ...
    try {
      ...
      await fetch('/api/notifications', {
        ...
        body: JSON.stringify({
          ...
          message: `Data penduduk atas nama ${data.name} telah diperbarui secara otomatis melalui pembuatan SKU.`,
          category: 'Residents'
        })
      });
    } catch (e) {
  ...
  const handlePrint = async (skipCheck = false) => {
    ...
    doc.write(`
      <html>
        <head>
          <title>Cetak SKU - ${formData.nama}</title>
          ${styles}
          <style>
    ...
    localStorage.setItem('riwayat_surat_sku', JSON.stringify(updatedRiwayat));
    setLoading(false);
    setSuccess(true);
  };
  ...
  const generateHTML = () => {
    ...
    return `
      ...
      <!-- DATA PENDUDUK -->
      <table style="width:calc(100% - 40px);border-collapse:collapse;margin-bottom:10px;margin-left:40px;line-height:1.4;font-size:14px;">
      ...
      <table style="width:calc(100% - 40px);border-collapse:collapse;margin-bottom:10px;margin-left:40px;line-height:1.4;font-size:14px;">
        <tr><td style="width:30%;">Nama Usaha / Toko</td><td style="width:3%;">:</td><td><strong style="text-transform:uppercase;">${v(formData.usahaName, 'WARUNG / TOKO PERORANGAN')}</strong></td></tr>
        <tr><td>Jenis / Bidang Usaha</td><td>:</td><td>${v(formData.usahaJenis, 'Perdagangan Umum')}</td></tr>
              ...
              <button 
                onClick={() => {
                  if (confirm('Kosongkan riwayat?')) {
                    setRiwayat([]);
                    localStorage.removeItem('riwayat_surat_sku');
                  }
                }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100/60">
                    ...
                    <div className="absolute right-0 mt-2 p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 shadow-2xl rounded-2xl z-50 w-72 animate-in fade-in zoom-in-95 duration-150">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {INDONESIAN_MONTHS.map((month, idx) => {
                          const isSelected = formData.usahaMulai === `${month} ${pickerYear}`;
                          return (
                            <button
                              key={idx}
                              type="button"
                              onClick={() => {
                                setFormData({...formData, usahaMulai: `${month} ${pickerYear}`});
                      ...
                      <div className="flex items-center justify-between mt-4 pt-2 border-t border-gray-100 dark:border-slate-800">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setFormData({...formData, usahaMulai: `${INDONESIAN_MONTHS[now.getMonth()]} ${now.getFullYear()}`});
                            setShowMulaiPicker(false);
                          }}
                ...
                <div className="space-y-2">
                  ...
                  <RupiahInput 
                    ...
                    onChange={(val) => setFormData(prev => ({ ...prev, usahaOmzet: val }))}
                  />
                </div>
              </div>
            </CardWrapper>

{/* Card 4: Keperluan Surat */}
            <CardWrapper title="Keperluan Surat" icon={FileText}>
              <div className="space-y-2">
