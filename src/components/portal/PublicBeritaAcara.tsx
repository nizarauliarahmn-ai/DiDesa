...
export default function PublicBeritaAcara({ onBack }: { onBack: () => void }) {
  const [ba, setBa] = useState<BeritaAcaraData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const baId = params.get('ba_id');
    if (baId) fetchBa(baId);
    else setLoading(false);
  }, []);

  const fetchBa = async (id: string) => {
    try {
      ...
      const baItems: BeritaAcaraData[] = all['berita_acara'] || [];
      const found = baItems.find((item) => item.id === id);
      setBa(found || null);
    } catch {
      setBa(null);
    } finally {
  ...
  const handleDownloadDocument = () => {
    if (!ba?.documentData) return;
    const byteCharacters = atob(ba.documentData);
    const byteNumbers = new Array(byteCharacters.length);
    for (let i = 0; i < byteCharacters.length; i++) byteNumbers[i] = byteCharacters.charCodeAt(i);
    const byteArray = new Uint8Array(byteNumbers);
    const blob = new Blob([byteArray], { type: ba.documentType || 'application/pdf' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = ba.documentName || 'Berita_Acara.pdf';
    a.click();
  ...
  if (!ba) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
        ...
        <p className="text-gray-500">Data Berita Acara tidak ditemukan.</p>
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-emerald-100 overflow-hidden">
          <div className="bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-4">
            ...
            <p className="text-emerald-100 text-xs mt-1">Dokumen resmi berita acara desa</p>
          </div>
          <div className="px-6 py-5 space-y-4">
            ...
            {ba.ketLain && (
              <div>
                ...
                <p className="text-sm text-slate-700">{ba.ketLain}</p>
              </div>
            )}
          </div>
          <div className="px-6 py-4 border-t border-gray-100 flex flex-wrap gap-3">
            {ba.linkFile && (
              <a href={ba.linkFile} target="_blank" rel="noopener noreferrer"
