...
# Use regex to replace both grids with new 4-column grid
$pattern = '(?s)<div className="grid grid-cols-2 gap-4">.*?<div className="grid grid-cols-1 md:grid-cols-3 gap-4">.*?</div>\s*</div>'
...
$content = Get-Content 'C:\Users\ASUS\.gemini\antigravity\scratch\DiDesa\src\components\admin\surat\AdminSuratSKTM.tsx' -Raw
$content = $content -replace '(?s)<div className="grid grid-cols-2 gap-4">.*?<div className="grid grid-cols-1 md:grid-cols-3 gap-4">.*?</div>\s*</div>', $new
