...
// POST chat with Desi (AI Assistant)
app.post("/api/chat-desi", async (req, res) => {
  ...
  try {
    ...
    // 1. Fetch village context (Residents & Tenant info)
    const tenants = await getTenants();
    const currentTenant = tenants.find(t => t.id === tenantId) || { name: "Desa Tidak Diketahui", domain: "-" };
    ...
    const topJobs = Object.entries(jobCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(j => `${j[0]}: ${j[1]}`).join(', ');
...
- Nama Desa: ${currentTenant.name} (Domain: ${currentTenant.domain})
- Total Penduduk: ${totalResidents} jiwa
- Laki-laki: ${male} jiwa
- Perempuan: ${female} jiwa
- 5 Profesi Terbanyak: ${topJobs || '-'}

Gunakan data di atas untuk menjawab pertanyaan terkait desa ini.`;

    // 3. Format messages for Gemini SDK
    const geminiMessages = messages.map(msg => ({
      role: msg.role === 'user' ? 'user' : 'model',
      parts: [{ text: msg.content }]
    }));

    // Insert System Prompt implicitly as first message (or configure it)
    // For @google/genai, system prompt goes into config.systemInstruction
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash", // faster model for chat
    ...
    const reply = response.text || "Maaf, saya tidak dapat memproses permintaan Anda saat ini.";
    res.json({ reply });
  } catch (err: any) {
    console.error("Error in chat-desi:", err);
    res.status(500).json({ error: err.message });
  }
});
