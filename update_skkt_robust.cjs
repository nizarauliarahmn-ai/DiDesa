...
if (handleSaveStart === -1 || handleSaveEnd === -1) {
  console.log('Error handleSave'); process.exit(1);
}

// Find preview start by finding the previewZoom div
const previewMarker = `style={{ transform: \`scale(\${previewZoom})\`, transformOrigin: 'top left', width: \`\${100 / previewZoom}%\` }}`;
let previewStart = content.lastIndexOf('<div ', content.indexOf(previewMarker));
if (previewStart === -1) {
    console.log('Error preview start'); process.exit(1);
}
// Find preview end by finding Map Picker Modal
let previewEndStr = '{/* Interactive Satellite Map Picker Modal */}';
let previewEnd = content.indexOf(previewEndStr);
// The preview block ends just before `      </div>\n\n      {/* Interactive`
// So we find `      </div>\n\n      {/* Interactive`
previewEnd = content.lastIndexOf('      </div>', previewEnd);

const generateHtmlCode = `  const v = (val, fallback = '-') => (val && val.trim() !== '' ? val : fallback);
...
content = content.substring(0, handleSaveStart) + generateHtmlCode + newHandleSave + content.substring(handleSaveEnd, previewStart) + newPreview + content.substring(previewEnd);
