...
file_path = r'C:\Users\Gambar Ibung\.gemini\antigravity\scratch\DiDesa\src\components\admin\surat\AdminSuratSKKT.tsx'
...
handle_save_start = find_line('const handleSave = async () => {')
handle_save_end = find_line('const todayStr = new Date().toLocaleDateString', handle_save_start) - 1

if handle_save_start == -1 or handle_save_end <= handle_save_start:
    print('Failed to find handleSave bounds')
    sys.exit(1)

preview_start = find_line('<div', find_line('className="w-full bg-white text-black p-10 shadow-xl rounded-lg font-serif border border-gray-300 overflow-x-auto select-none"')) - 1
preview_end = find_line('</div>', find_line('<!-- TANDA TANGAN -->', preview_start)) + 4

if preview_start == -1 or preview_end <= preview_start:
    print('Failed to find preview block bounds')
    sys.exit(1)
...
new_lines = lines[:handle_save_start] + [generate_html] + lines[handle_save_end:preview_start] + [new_preview] + lines[preview_end:]
